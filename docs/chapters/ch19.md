...
> By activating only a few experts per input, expert parallelism reduces per-device memory, inference time, and compute costs for models with a large number of experts, often called wide expert models. The conditional, router-based expert compute pattern scales efficiently as you add more experts.

> For instance, DeepSeek-R1 has 256 total experts, but only the top 9 experts (including 1 shared expert) are chosen by the router during inference.
...
> Let's consider an example. If we have an 8x GPU Blackwell B200 server totaling 1,440 GB of HBM memory (1,440 GB = 180 GB per GPU x 8 GPUs), we can serve DeepSeek-R1 with four-way TP across four GPUs-leaving the other four GPUs idle.

> If a single query arrives with an extremely long context (e.g., > 1 million tokens), the scheduler can spawn a two-stage pipeline such that stage 1 spans GPUs 0-3 and stage 2 spans GPUs 4-7. This effectively doubles the available GPU memory per stage to ~720 GB (180 GB per GPU x 4 GPUs) of total HBM (720 GB usable HBM). This helps avoid OOM errors when processing large inputs.
...
> Combining 4-bit weight quantization with 8-bit activations can reduce memory significantly. For instance, pure compute-limited kernels FP8 activations can achieve up to 2x throughput-especially on high-bandwidth modern GPUs. For mixed or memory-bound workloads, 1.5x is achievable.

> Using FP4 for activations can push memory savings even further. However, it may introduce slightly higher cumulative error that requires careful layer-wise tuning.
...
> INT4 weight + FP8 activation workflows can produce about ~1.8x of the baseline throughput when memory is the main bottleneck. INT4 weights + FP4 activations can reduce memory down to 20% of the baseline. 4-bit targets. The speedup is around 3.5x, which is consistent with the theoretical peak 4x improvement over FP16.
...
> Choosing a tiled attention algorithm should balance data reuse against SM occupancy. For instance, consider T as your tile width, in tokens, per thread block. Each thread block reserves on the order of O(T2) floats in shared memory to hold the query, key, and value chunks since self-attention is quadratic in nature.
...
> Here, T is computed based on the sequence length L, the GPU's per-thread-block shared-memory limit, gpu_shared_mem_per_block, and the SM count, num_sms. Then, the per-thread-block shared memory, shared_mem_bytes, is computed at runtime based on the computed tile size, T.
...
> You should also continuously monitor SM utilization metrics. Consider many idle warps (e.g., < 50% active warps) or memory stall cycles (> 70% stalled). This indicates an imbalance, as either occupancy is too low (idle warps)-or your tile size is too small and causes excessive memory traffic.

> As such, your system should adjust accordingly to restore the balance.
...
> RL-based tuning involves formulating the inference serving setup as a Markov Decision Process (MDP). The system state includes current workload metrics (e.g., queue length, GPU memory usage, number of active requests, average sequence length, etc.) and hardware status (e.g., temperature, power consumption, SM occupancy). Actions include adjusting parameters like batch size, parallelism strategy, precision level, or kernel selection. The reward is the objective function, such as throughput minus latency penalty, or percentage of requests meeting SLOs.

> At each decision interval, the agent observes the current state and selects an action. The system then executes the inference workload for a period. Afterward, the agent receives the reward and observes the new state. The agent updates its policy to maximize cumulative reward.

> This approach can learn complex nonlinear policies. For instance, the agent might learn to increase batch size when the queue is long and GPU memory is ample. It might learn to decrease batch size when latency approaches the SLO limit. It might learn to switch to pipeline parallelism when long sequences are detected.

> A practical example is using Deep Q-Network (DQN) or Proximal Policy Optimization (PPO) to learn a batch-size policy. The state includes queue length, current batch size, average latency, and GPU utilization. The action is to increase, decrease, or maintain batch size. The reward is throughput minus latency penalty.

> RL-based tuning can adapt to changes in workload distribution. If traffic patterns change-for example, from short prompts to long prompts-the agent automatically learns the new optimal policy. This is more robust than static configuration.

> However, RL requires careful design. The reward function must accurately reflect the objectives. The exploration strategy must balance trying new configurations with exploiting known good configurations. Training time can be long, so it's common to train first in simulation or on offline data, then fine-tune online.

> It's recommended to train the RL agent offline using historical workload data before deployment. Then use online learning to fine-tune. This reduces the risk of the exploration phase in production.

> RL-based tuning can also be applied to other decisions, such as kernel selection. The agent can learn to choose the best kernel variant for specific input sizes. Or it can learn when to enable speculative decoding or KV cache compression.

> Advanced systems use multi-agent RL, where different agents control different subsystems. One agent might control batch size, another precision, and a third caching policy. The agents coordinate to optimize the global objective.

> RL-based tuning is an active research area. It promises to make inference systems self-optimizing, reducing the need for manual tuning. As LLMs become more complex and workloads more diverse, this adaptive capability will become increasingly important.
...
> It's recommended to implement allocator monitoring for long-running inference services. Track memory fragmentation rate, allocation latency, and OOM events. When metrics exceed thresholds, consider switching allocators or restarting the process.

动态内存分配器管理是一个高级主题。它需要对GPU内存行为有深入了解。然而，对于大规模推理服务，它可以是优化性能和稳定性的有力工具。

> Dynamic memory allocator management is an advanced topic. It requires deep understanding of GPU memory behavior. However, for large-scale inference services, it can be a powerful tool for optimizing performance and stability.
...
> Over time, dynamic adjustments like this keep communication efficient-even in the face of varying batch sizes, input data distributions, and other dynamic workload changes. The system learns congestion patterns and adapts quickly by modifying schedules based on real-time feedback. The system can use an RL agent (discussed in earlier sections) or a set of heuristic rules. This is critical for environments with bursty and unpredictable inference requests.

### 使用精细调度协调NVSwitch传输

NVLink/NVSwitch系统的核心是NVSwitch结构本身。这是一个处理许多同时GPU到GPU传输的集中式交叉开关。NVSwitch具有极高的带宽，并有自己的内部调度算法，包括跨多个交换机芯片和平面的自适应路由。

> At the heart of the NVLink/NVSwitch system is the NVSwitch fabric itself. This is a centralized crossbar that handles many simultaneous GPU-to-GPU transfers. NVSwitch is extremely high-bandwidth and has its own internal scheduling algorithms, including adaptive routing across multiple switch chips and planes.
...
> Consider all 72 GPUs in an NVL72 rack needing to send data to a particular peer-for instance, a centralized parameter server where GPU 0 gathers all results from all 72 GPUs. If all 71 other GPUs send data at the exact same time, GPU 0's 18 NVLink links-and the NVSwitch that connects them-will experience a massive burst of 71 incoming transfers, as shown in Figure 19-14. This would exceed the amount of bandwidth that can be provided at that moment.
...
> In this situation, the NVSwitch would need to buffer and serialize many of these transfers. This causes latency spikes. Instead, a coordinated and optimized approach is to split the senders into four groups: group 1 (GPUs 1-18) sends first, then a few microseconds later group 2 (GPUs 19-36) sends, and so on.
...
> Input-Aware Layer Skipping (DASH): Frameworks like DASH present inference as a Markov Decision Process, which dynamically decides, per-token, whether to execute or skip each transformer layer based on input characteristics. By learning a small scoring network, DASH can skip 20%-40% of layers for many tokens.
...
> Speculative MoE Expert Routing and Communication Reduction: For MoE models, speculative expert routing anticipates which experts will be activated for upcoming tokens and co-shuffles tokens and expert assignments ahead of time.
...
通过根据输入在运行时选择正确的变体，可以避免"一刀切"方法带来的巨大性能悬崖。在实践中，你可以在目标硬件上进行基准测试，找出大约 L = 128 是盈亏平衡点。
...
接下来，让我们在自动调优的背景下分析前馈MLP内核。前馈层本质上是大型矩阵乘法——具体来说，是两个线性投影，中间有一个非线性激活函数。
...
现代AI框架如PyTorch使用高度优化的GEMM内核，这些内核使用优化的CUDA库，如cuBLAS和CUTLASS。在这些库中，对于给定的矩阵大小，通常有多种算法变体，它们使用不同的分块策略、不同的Tensor Core和单独的回退路径。
...
例如，NVIDIA的cuBLAS和cuBLASLt库可以通过首先尝试几种算法，然后为给定维度选择最快的算法来自动调优GEMM内核。但是，这通常发生在第一次遇到该形状的GEMM时，之后不会再重新访问。
...
在cuBLAS/cuBLASLt或自定义内核中可用的情况下，程序化依赖启动（PDL）可以减少启动间隙并提高稳态吞吐量。请确保进行分析以确认重叠。
...
在处理许多不同批大小的推理服务器中，可以显式调用此类自动调优机制，或维护最佳算法的缓存。例如，对于形状为 [batch_size, hidden_dim] x [hidden_dim, 4*hidden_dim] 的MLP的GEMM，batch_size = 1 与 batch_size = 16 的最优内核可能不同。引擎可以检测新的批大小，并使用cuBLASLt或自定义实现对候选内核运行快速微基准测试，以选择最快的内核。随后使用该批大小的调用可以直接使用所选的内核。

> In an inference server that sees many different batch sizes, one can explicitly invoke such autotuning mechanisms-or maintain a cache of best algorithms. For instance, for the MLP's GEMM of shape [batch_size, hidden_dim] x [hidden_dim, 4*hidden_dim], the optimal kernel might differ for batch_size = 1 versus batch_size = 16. The engine can detect a new batch size and run a quick microbenchmark of candidate kernels using cuBLASLt or a custom implementation to select the fastest kernel. Subsequent calls with that batch size can then directly use the chosen kernel.

此外，一些推理框架和运行时使用OpenAI的Triton GPU内核领域特定库（DSL）来即时编译具有自动调优分块大小的注意力和MLP内核。在这种情况下，运行时会生成几个具有不同分块大小（例如，128 x 128、64 x 256等）的内核变体，并根据实际硬件和输入形状测量哪个性能更好。

> In addition, some inference frameworks and runtimes use OpenAI's Triton GPU kernel domain-specific library (DSL) to compile attention and MLP kernels on the fly with autotuned tile sizes. In this case, the runtime would generate a few variants of a kernel with different tile sizes (e.g., 128 x 128, 64 x 256, etc.) and measure which performs better given the actual hardware and input shape.

你可以使用Nsight Systems等工具并排分析不同的内核变体。具体来说，Nsight Systems提供详细的CUDA时间线，包括memcpy和NVLink活动，而Nsight Compute提供内存工作负载分析，帮助将缓存和内存行为归因到内核位置。这在评估分块大小和共享内存权衡时特别有用。此外，它通常可以揭示非显而易见的瓶颈，如L2缓存未命中，这将进一步指导你的调优决策。

> You can use tools like Nsight Systems to empirically profile different kernel variants side by side. Specifically, Nsight Systems provides detailed CUDA timelines, including memcpy and NVLink activity, and Nsight Compute provides memory workload analysis that helps attribute cache and memory behavior to kernel sites. This is particularly useful when evaluating tile-size and shared-memory trade-offs. In addition, it can often reveal nonobvious bottlenecks like L2 cache misses that will further guide your tuning decisions.

由于硬件在负载下有时可能有些不可预测，考虑到L2缓存效应、内存bank冲突等因素，经验调优总是胜过理论猜测。但最好以合理的理论值开始调优过程。
...
动态分块切换会影响GPU占用率，在选择分块大小时应予以考虑。使用较大的分块可以增加重用并减少内核启动开销，但也可能使用更多的寄存器和共享内存。这可能会减少可以并发运行的线程块数量——从而降低占用率。一个合适的自动调优器会考虑这种权衡。
...
在注意力内核中，较大的分块（例如，128 x 128）可以最大化共享内存中的数据重用。这对于长序列是理想的，因为你发出更少的全局内存加载，分摊循环开销，并产生更高的持续吞吐量。

> In attention kernels, a larger tile (e.g., 128 x 128) maximizes data reuse in shared memory. This is ideal for long sequences since you issue fewer global-memory loads, amortize loop overhead, and produce higher sustained throughput.

然而，对于较短的序列，同样的大分块可能会消耗过多的共享内存，这限制了占用率，或每个SM上并发运行的线程块数量。通过为较短的序列减小分块大小（例如，64 x 64），你可以释放共享内存，以便可以并行调度更多的块。这提高了SM占用率并减少了每个内核的延迟。

> For shorter sequences, however, that same large tile can consume too much shared memory, which limits the occupancy, or number of concurrent thread blocks running on each SM. By reducing the tile size (e.g., 64 x 64) for shorter sequences, you free up shared memory so you can schedule more blocks in parallel. This boosts SM occupancy and reduces per-kernel latency.

通过根据输入序列长度调整分块大小，内核在大多数情况下可以实现接近最优的占用率。一些系统甚至在运行时查询CUDA Occupancy API来动态选择内核启动参数，例如线程块大小。接下来展示了一个C++中Occupancy API的示例，但Python API也可用：
...
这段伪C++代码说明了如何为内核评估不同的线程块大小。它检查在给定共享内存使用情况下，每个SM可以运行多少个块。然后内核启动会相应地调整线程数量或共享内存使用。高性能框架和推理引擎在内部使用以下步骤自动执行此类逻辑：
...
3. **估计或基准测试**：通过在样本数据上执行每个算法几次来快速运行每个候选算法。
...
4. **选择最佳变体**：根据测量的维度选择执行时间最短或吞吐量足够的内核。
...
5. **缓存结果**：将选择存储在以输入维度或工作负载签名为键的查找表中。这样，如果出现类似的请求，无需重新运行这些步骤就知道最佳内核。
...
这类似于数据库查询优化器选择查询计划。在这里，"计划"就是所选的内核实现。

> This is analogous to a database query optimizer picking a query plan. Here, the "plan" is the chosen kernel implementation.

通过遵循这样的过程，推理运行时会持续自我调优。随着时间的推移，系统为各种场景构建优化路径库，例如短提示与长提示、小批与大批等。即时调优的开销通过异步执行——在单独的流中测试新内核，而当前推理使用默认内核——或在低流量期间执行来保持较低，以免影响延迟。
...
建议在加载模型时加入初始预热阶段，通过运行各种样本输入来触发自动调优。这可以包括极端情况——如最大序列长度、最大批大小等——以便引擎为这些情况预先优化内核。
...
此外，最好在运行时监控每层的执行时间。如果某层由于输入特征的变化突然成为瓶颈，那么是时候重新审视内核选择了。
...
一些高级框架甚至使用多臂老虎机算法，持续探索替代内核，并在条件变化时更新不同内核的选择。
...
简而言之，自动调优将静态内核转变为自适应内核。这为每组输入从GPU集群中榨取最高性能，无论工作负载如何。你可以确信系统在不断适应。
