...
![图19-1 PP引起的流水线气泡](../assets/images/ch19/fig19_01.png)
...
![图19-2 为给定模型跨八个GPU预配置两种不同的混合分片池（TP = 4, PP = 1 和 TP = 2, PP = 1）](../assets/images/ch19/fig19_02.png)
...
让我们考虑一个例子。如果我们有一个8× GPU Blackwell B200服务器，总共1,440 GB的HBM内存（1,440 GB = 180 GB每GPU × 8个GPU），我们可以跨四个GPU使用四路TP服务DeepSeek-R1——让其他四个GPU空闲。

如果单个查询到达且具有极长的上下文（例如，> 100万个token），调度器可以生成一个两阶段流水线，使得阶段1跨越GPU 0-3，阶段2跨越GPU 4-7。这有效地将每个阶段的可用GPU内存加倍到约720 GB（180 GB每GPU × 4个GPU）的总HBM（720 GB可用HBM）。这有助于在处理大输入时避免OOM错误。
...
此外，一些推理框架和运行时使用OpenAI的Triton GPU内核领域特定库（DSL）来即时编译具有自动调优瓦片大小的注意力和MLP内核。在这种情况下，运行时会生成具有不同瓦片大小（如128 × 128、64 × 256等）的内核的几个变体，并测量在给定实际硬件和输入形状下哪个表现更好。
...
在注意力内核中，更大的瓦片（如128 × 128）最大化共享内存中的数据重用。这对于长序列是理想的，因为您发出更少的全局内存加载，分摊循环开销，并产生更高的持续吞吐量。

然而，对于较短的序列，同样的大瓦片可能消耗太多共享内存，这限制了占用率，或每个SM可以并发运行的线程块数量。通过为较短序列减小瓦片大小（如64 × 64），您可以释放共享内存，以便可以并行调度更多块。这提高了SM占用率并减少了每个内核的延迟。
...
![图19-3 使用SpeCache的推测性解码](../assets/images/ch19/fig19_03.png)
...
![图19-4 不同的KV缓存算法，包括无缓存（如密集）](../assets/images/ch19/fig19_04.png)
...
![图19-5 滑动窗口注意力使用最有用的token是最新的这一直觉](../assets/images/ch19/fig19_05.png)
...
![图19-7 分块预填充对四个请求的解码最大化批处理的好处](../assets/images/ch19/fig19_07.png)
...
![图19-8 拓扑感知路由以避免跨GPU和多节点集群的瓶颈](../assets/images/ch19/fig19_08.png)
...
![图19-9 NVIDIA拓扑感知GPU选择（NVTAGS）进程到GPU映射](../assets/images/ch19/fig19_09.png)
...
![图19-10 使用多个GPU和跨NVLink全归约的分布式GEMM](../assets/images/ch19/fig19_10.png)
...
![图19-11 基于树（链）与基于环的通信](../assets/images/ch19/fig19_11.png)
...
作为单个整体步骤执行此交换意味着每个GPU同时向71个其他GPU发送数据。这是72个GPU × 71条消息同时使每个链路和NVSwitch端口饱和。
...
![图19-12 使用GPUDirect RDMA的直接GPU到GPU内存传输——不涉及主机CPU内存](../assets/images/ch19/fig19_12.png)
...
![图19-13 网络级数据包路由以避免拥塞](../assets/images/ch19/fig19_13.png)
...
![图19-14 所有72个GPU向中央参数服务器发送数据](../assets/images/ch19/fig19_14.png)
...
**使用torch.compile的稳态推理**
如果可以承受预热时间，优先选择mode="reduce-overhead"或autotune模式。这将有助于最小化低延迟推理工作负载的运行时开销。

**内核级自动调优**
动态优化GPU内核和瓦片大小。尽可能利用张量内存加速器（TMA）进行异步内存预取。使用提供自动调优的库和编译器，如CUTLASS和Triton——而不是手动调优它们，除非为了性能绝对必要。

**自适应精度**
在推理期间在8位和4位浮点（FP8/FP4）之间切换，以平衡速度和精度。您也可以根据需要与16位精度混合。在PyTorch中使用Transformer Engine进行FP8，因为torch.autocast()不直接支持FP8。

**解耦推理流水线**
跨资源分离预填充（提示处理）和解码（生成）阶段，以及基于实时因素（如KV缓存命中、队列深度和负载）的上下文感知请求路由。这保持了高吞吐量和长提示——而不会减慢短提示响应。

**动态并行策略**
根据输入/输出序列长度和模型结构——包括MoE路由——在数据并行、张量并行、流水线并行和混合执行组合之间即时决策。这些决策包括复制或分片模型。

**自适应解码和调度**
使用推测性解码、飞行中批次重塑和token级调度等技术来提高吞吐量和延迟。这些技术在vLLM、SGLang和NVIDIA TensorRT-LLM等引擎中实现。这验证了它们在生产环境中的有效性。

**内存管理和统一内存调优**
利用Grace CPU的内存和统一寻址将不常用的KV缓存页面卸载到CPU或NVMe。使用cudaMemAdvise和cudaMemPrefetchAsync等API进行最佳放置。确保使用GPUDirect Storage（如果可用）。这将直接从NVMe分页数据到GPU内存——绕过CPU。
