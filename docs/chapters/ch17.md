...
![图17-3 预填充工作器设计：读取前缀缓存 → 计算预填充 → 为解码写入KV缓存](../assets/images/ch17/fig17_03.png)
...
![图17-4 使用NIXL在预填充和解码工作器之间传输KV缓存数据；在RDMA之前将多个PagedAttention块合并为约128个token的有效负载（注意：vLLM在CUDA上默认每块16个token）](../assets/images/ch17/fig17_04.png)
...
![图17-5 在请求之间管理和重用KV缓存数据](../assets/images/ch17/fig17_05.png)

> Figure 17-5. Managing and reusing KV cache data between requests

对于大型模型和长序列，KV内存随token线性增长，并取决于注意力布局和数据类型。一个实用的估算是bytes_per_token = 2 × n_layers × n_kv_heads × head_dim × bytes_per_element。（注意：2×考虑了每层每token的键和值。）
...
![图17-6 基于从工作器发出的KV缓存事件接收的数据进行KV缓存感知请求路由](../assets/images/ch17/fig17_06.png)
...
![图17-7 NVIDIA的Dynamo GPU Planner根据GPU利用率指标决定如何处理传入请求并将GPU工作器分配给预填充和解码](../assets/images/ch17/fig17_07.png)
