...
![图17-2 共置和分离式预填充和解码的比较，显示分离设置中每秒请求数更高，实现更高的有效吞吐量](../assets/images/ch17/fig17_1702.png)
...
![图17-3 预填充工作器设计：读取前缀缓存 → 计算预填充 → 为解码写入KV缓存，详细说明了在分布式系统中管理预填充请求和KV块操作的过程](../assets/images/ch17/fig17_1703.png)
...
![图17-4 使用NIXL在预填充和解码工作器之间传输KV缓存数据，展示了调度和远程内存操作，在RDMA之前将多个PagedAttention块合并为约128个token的有效负载（注意：vLLM在CUDA上默认每块16个token）](../assets/images/ch17/fig17_1704.png)
...
![图17-5 在不同请求之间管理和重用KV缓存数据，显示了从缓存中的拉取、编码和生成过程，存储在GPU、CPU和SSD缓存中](../assets/images/ch17/fig17_1176_1.png)
...
![图17-6 基于从工作器发出的KV缓存事件接收到的数据的KV缓存感知请求路由，显示了从聊天/代理请求通过分析、token化和缓存匹配到选择工作器节点进行优化数据处理的流程](../assets/images/ch17/fig17_1706.png)
