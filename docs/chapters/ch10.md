...
简而言之，将CUDA Pipeline API与协作组一起使用，可以实现细粒度、SM范围的生产者-消费者传递，而无需任何显式的`__syncthreads()`调用。表10-1比较了三种实现：朴素分块内核、使用`double_buffered_pipeline`的两阶段双缓冲GEMM，以及我们的线程束专用流水线内核`warp_specialized_pipeline`。

**表10-1. 三种实现的比较：朴素分块内核、两阶段双缓冲GEMM和线程束专用流水线内核**
...
> Table 10-1. Comparison of three implementations: a naive tiled kernel, a two-stage double-buffered GEMM, and a warp-specialized pipeline kernel
...
如表10-1所示，双缓冲和线程束专用方法都大大优于朴素分块内核。`double_buffer_pipeline`通过重叠块加载和计算将运行时间减半，而`warp_specialized_pipeline`通过避免任何隐式的块范围等待增加了额外的10.2%加速。只有专用的"计算"线程束会停顿。
...
此外，一种源自大规模推理的现代持久内核方法称为兆内核。兆内核将跨层——甚至跨GPU——的整个操作序列融合为单个大型内核。如图10-1所示，持久兆内核已显示出通过消除重复的内核启动开销，将延迟比传统的每层启动降低1.2倍到6.7倍。

![图10-1 使用兆内核相对于vLLM和SGLang的解码吞吐量改进（来源：https://oreil.ly/2aZiF）](images/ch10/fig10-1.png)

> Figure 10-1. Decode throughput improvement with megakernels relative to vLLM and SGLang (source: https://oreil.ly/2aZiF)
...
协作组让您以任意粒度定义和同步线程组。例如，您可以创建具有单个线程、线程束、tile、块和集群的组，如图10-2所示。

![图10-2 使用协作组跨不同粒度的线程进行同步](images/ch10/fig10-2.png)

> Figure 10-2. Synchronizing across different granularities of threads using cooperative groups
...
这个单一内核取代了原本需要2 × 1,000 = 2,000次内核启动的操作，并避免了重复的启动开销。在循环内部，`grid.sync()`确保正确的顺序（每个块在任何块开始阶段2之前完成阶段1——以及在下一次迭代之前），无需任何主机端同步。
...
相比之下，线程块集群（或协作线程阵列[CTA]集群）是一个硬件级层次结构。它为您的协作网格授予一部分SM——并将剩余部分留给其他内核使用。这减轻了一个内核独占GPU的风险。GPU保证线程块将在同一个GPU处理集群（GPC）上共同调度，如图10-3所示。

![图10-3 多个线程块集群保证在同一个GPC或GPC分区上共同调度](images/ch10/fig10-3.png)

> Figure 10-3. Multiple thread block clusters are guaranteed to be coscheduled on the same GPC or GPC partition
...
此集群级屏障让您只同步一部分线程块而不阻塞整个GPU。线程块集群让您启动一个协作内核，将网格细分为更小的块组，如图10-4所示。

![图10-4 对于这四个（2×2）线程集群，A和B的每个块同时加载到两个线程块中（来源：https://oreil.ly/kEZsv）](images/ch10/fig10-4.png)

> Figure 10-4. For these four (2 × 2) thread clusters, each tile of A and B is loaded into two thread blocks simultaneously (source: https://oreil.ly/kEZsv)
...
为了解决这种低效问题，您可以使用线程块搅动。与使用搅动优化内存访问和避免共享内存bank冲突类似，您可以使用线程块搅动避免以低效的行优先和列优先顺序分配块，如图10-5所示。

![图10-5 线程块搅动以在单个波中从L2缓存读取块A和B](images/ch10/fig10-5.png)

> Figure 10-5. Thread block swizzling to read tiles A and B in a single wave out of L2 cache
...
现代NVIDIA GPU让您在单个GPC内的SM之间共同调度恰好两个线程块在一个集群中，或线程块对（又名CTA对）。通过将线程块分组在一个集群中（例如，如图10-6所示的2块集群），共享数据的内核可以使用TMA将块移动到每个块的共享内存中。

![图10-6 线程块对组合两个线程块](images/ch10/fig10-6.png)

> Figure 10-6. Thread block pair combines two thread blocks

单个线程块可能缺乏寄存器或共享内存容量来独自处理非常大的块（例如，256 × 256矩阵子块）。通过在GPC内的附近SM上配对两个线程块并使用DSMEM，这两个块可以在一个大型块上分割工作，但通过统一的共享内存区域共享数据，如图10-7所示。

![图10-7 线程块对（又名CTA对）加载块作为A * B矩阵乘法的操作数（来源：https://oreil.ly/kEZsv）](images/ch10/fig10-7.png)

> Figure 10-7. Thread block pair (aka CTA pair) loading tiles as operands for an A * B matrix multiply (source: https://oreil.ly/kEZsv)

在这里，对中的每个线程块可以将操作数块的一部分（例如128 × 16）加载到其片上SMEM中用于矩阵乘法。此外，每个线程块在张量内存（TMEM）中保存累加器的一部分（例如128 × 256）。这允许CTA对中的两个线程块在单个块上协作，如图10-8所示。

![图10-8 带有Tensor Core和TMEM的线程块对](images/ch10/fig10-8.png)

> Figure 10-8. Thread block pair with Tensor Cores and TMEM

线程块对允许跨越两个物理SM的更大矩阵乘法。使用一对线程块有效地使块大小加倍，因为每个SM处理块数据的一半。

SM硬件使用DSMEM在SM之间共享操作数数据。DSMEM减少重复加载，改善数据重用，并增加算术强度。图10-9显示了使用DSMEM在线程块集群中的SM之间共享数据。

![图10-9 使用DSMEM在线程块集群中的SM之间共享数据](images/ch10/fig10-9.png)

> Figure 10-9. Sharing data between SMs in a thread block cluster using DSMEM
...
如果您请求单个线程块无法处理的块大小（例如，FP16 Tensor Core操作的128 × 128），CUTLASS将自动分配两个线程块作为一对。SM分割工作并在底层依赖DSMEM + `cluster.sync()`来共享块。这样，您可以成对重叠UMMA操作与DSMEM。
...
使用TMA多播，GPU确保L2缓存数据在一个过程中广播到每个集群成员（线程块）的共享内存中。因此，您避免了同一块的重复全局内存加载。这改善了带宽利用率并减少了DRAM流量——尤其是当许多块需要相同输入时，如图10-10所示。

![图10-10 对于这四个（2×2）线程集群，每个块加载一次并多播到每个集群中所有CTA的共享内存中（来源：https://oreil.ly/kEZsv）](images/ch10/fig10-10.png)

> Figure 10-10. For these four (2 × 2) thread clusters, each tile is loaded once and multicast into the shared memory of all CTAs in each cluster (source: https://oreil.ly/kEZsv)
...
考虑两个线程块CTA 0和CTA 1，它们都需要矩阵A的同一块。没有DSMEM，每个CTA发出自己的全局内存加载，这通过获取相同的数据两次浪费了DRAM带宽。然而，使用DSMEM，CTA 0将块加载到其共享内存一次，然后通过片上DSMEM网络多播，以便CTA 1可以直接从共享内存读取它。如果超过两个CTA形成一个集群，同一块将在集群中的所有线程块之间共享，但这仍然只需要一次全局HBM加载。表10-2显示了使用两个线程块的集群与两个独立线程块的比较。

**表10-2. DSMEM对两块工作负载的性能影响**

| 指标 | 两个独立CTA（无DSMEM） | 带DSMEM的CTA对（2个集群） |
|------|------------------------|--------------------------|
| 全局加载事务 | 2×（每个线程块加载块） | 1×（块加载一次） |
| L2缓存命中率 | 50% | 85% |
| 块间数据重用 | 无（无重用） | 显著（CTA 1重用块） |
| 每CTA有效DRAM带宽 | 300 GB/s | 150 GB/s（减少50%） |
| 内核时间（相对） | 1.0× | 0.6×（加速40%） |
...
重要的是要注意，DSMEM和L2并行运行。这为块间数据共享提供了两条"通道"。这种双路径设计将DSMEM的超快集群范围通信与L2更广泛的缓存覆盖相结合。换句话说，DSMEM访问绕过集群本地地址的L2缓存。相反，DSMEM使用专用的SM到SM网络，如图10-11所示。

![图10-11 DSMEM使用两个线程块集群之间的SM到SM线程块集群本地网络进行数据交换](images/ch10/fig10-11.png)

> Figure 10-11. DSMEM uses an SM-to-SM thread block cluster-local network between two thread block clusters for its data exchange
...
// 从DSMEM源计算TILE_SIZE×TILE_SIZE乘积的一个行带。
...
这允许块加载、计算和存储跨多个线程块交错，这将SM驱动到更高的利用率。表10-3比较了单块线程束专用内核与本节介绍的线程块集群（多块）线程束专用内核。

**表10-3. 朴素分块、两阶段双缓冲、线程束专用和线程块集群流水线内核的比较**
