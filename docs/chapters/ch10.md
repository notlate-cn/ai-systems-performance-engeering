...
![图10-11 对于这四个（2×2）线程集群，A和B的每个块同时加载到两个线程块中（来源：https://oreil.ly/kEZsv）](images/ch10/fig10-11.png)
...
单个线程块可能缺乏寄存器或共享内存容量来独自处理非常大的块（例如，256 × 256矩阵子块）。通过在GPC内的附近SM上配对两个线程块并使用DSMEM，这两个块可以在一个大型块上分割工作，但通过统一的共享内存区域共享数据，如图10-14所示。
...
在这里，对中的每个线程块可以将操作数块的一部分（例如128 × 16）加载到其片上SMEM中用于矩阵乘法。此外，每个线程块在张量内存（TMEM）中保存累加器的一部分（例如128 × 256）。这允许CTA对中的两个线程块在单个块上协作，如图10-15所示。
...
![图10-17 对于这四个（2×2）线程集群，每个块加载一次并多播到每个集群中所有CTA的共享内存中（来源：https://oreil.ly/kEZsv）](images/ch10/fig10-17.png)
