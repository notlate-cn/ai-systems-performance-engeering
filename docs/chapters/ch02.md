...
*图2-1 NVIDIA Grace Blackwell超级芯片模块，包含一个Grace CPU（中心）和两个Blackwell B200 GPU（左上和右上），在单个模块上具有共享的统一内存空间，通过称为NVLink-C2C（芯片到芯片）的定制高速链路连接*
...
虽然本节深入探讨双裸片架构的细节，但本书的其余部分将Blackwell的两个组合GPU裸片统称为"Blackwell GPU"。
...
由于纠错码（ECC）、系统固件使用、制造限制以及其他阻止芯片暴露完整192 GB的问题，每个B200的192 GB HBM3e内存中只有180 GB可用。因此，我们将使用180 GB而不是完整的192 GB来表示Blackwell B200的可用内存。
...
*图2-4 比较CPU核心与GPU核心（来源：https://oreil.ly/003EH, https://oreil.ly/Z25Tf）*
...
*图2-6 GB200/GB300 NVL72机柜内的1U计算托盘，包含两个Grace Blackwell超级芯片（来源：developer.nvidia.com）*
...
*图2-7 NVL72内部的一个NVLink交换托盘（来源：https://oreil.ly/h7seG）*
...
*图2-8 NVL72机柜内的九个托盘NVSwitch系统（来源：https://oreil.ly/h7seG）*
...
### 2.6.3 使用NVIDIA SHARP的网络内聚合 (In-Network Aggregations with NVIDIA SHARP)
...
### 2.12.4 Feynman GPU（2028）和每年翻倍某些东西 (Feynman GPU (2028) and Doubling Something Every Year)
