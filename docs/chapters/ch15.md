...
![图15-1 分离式预填充和解码共享KV缓存，并将KV传输与计算重叠](../assets/images/ch15/fig15_01.png)
...
![图15-2 分离式推理：预填充池（隐藏状态 + KV）→ 使用NVLink/NVSwitch（节点内）或GPUDirect RDMA（节点间）进行KV交接 → 解码池](../assets/images/ch15/fig15_02.png)
...
![图15-3 vLLM推理引擎中使用NIXL的KV缓存数据传输；节点内：NVLink/NVSwitch（设备到设备）；节点间：GPUDirect RDMA（InfiniBand/RoCEv2）使用ConnectX级别NIC](../assets/images/ch15/fig15_03.png)
...
![图15-4 使用开源llm-d项目的基于Kubernetes的vLLM集群，包含独立的预填充和解码工作节点；变体自动缩放器根据提示-响应混合调整预填充/解码副本数量；使用LMCache和NIXL移动KV](../assets/images/ch15/fig15_04.png)
...
![图15-5 模型权重和输入数据在GPU上的分割方式](../assets/images/ch15/fig15_05.png)
...
![图15-6 混合专家（MoE）通信（来源：https://oreil.ly/pzn5t）](../assets/images/ch15/fig15_06.png)
...
![图15-7 4 × 2 × EP混合并行组合的高级示意图（来源：https://oreil.ly/q1AEf）](../assets/images/ch15/fig15_07.png)

> Figure 15-7. High-level diagram of a 4 × 2 × EP hybrid parallel combination (source: https://oreil.ly/q1AEf)

在这里，我们使用四个流水线阶段（每个GPU一个）和两路张量并行。token使用专家并行跨两个专家路由。这被称为4 × 2 EP混合并行策略。
...
![图15-8 使用草稿（小）模型进行解码和目标（大）模型进行多token验证的推测解码](../assets/images/ch15/fig15_08.png)
...
![图15-9 使用EAGLE-2的推测解码（来源：https://oreil.ly/uG07b）](../assets/images/ch15/fig15_09.png)
...
![图15-10 使用Medusa进行多token解码（来源：https://oreil.ly/MJMOQ）](../assets/images/ch15/fig15_10.png)
...
在生产应用中，LLM通常需要生成结构化输出，如JSON、XML或遵循特定语法规则的代码。约束解码（Constrained decoding）是一种在生成过程中强制执行这些约束的技术，同时保持可接受的性能。

约束解码的核心思想是在每个解码步骤中限制模型只能从有效的token子集中选择。例如，如果输出必须是有效的JSON，则解码器会动态构建一个掩码，仅允许在当前位置有效的token。这确保生成的输出从一开始就符合所需的格式。

约束解码的主要方法包括：

**基于语法的方法**：定义形式语法（如JSON模式或正则表达式），并使用解析器状态机在每个位置确定有效token。例如，如果当前状态期望属性名，则只允许字符串token。

**基于有限状态机（FSM）的方法**：将语法约束编译成FSM，跟踪当前状态并为每个位置生成token掩码。这种方法由Outlines等库使用，可以高效处理复杂语法。

**基于Token树的方法**：预计算所有有效token序列的树结构，并在解码期间遍历这棵树。这对于固定词汇表特别有效。

约束解码的主要挑战是性能开销。在每个步骤构建和应用token掩码会增加计算开销。然而，现代实现通过以下方式最小化这种开销：

- 预编译语法到优化的数据结构
- 使用高效的位掩码操作
- 缓存中间状态以避免重复计算

在实践中，约束解码通常只增加很小的延迟开销（通常在5-15%范围内），同时显著提高输出可靠性和减少后处理需求。

> Constrained decoding is a technique used in production applications where LLMs often need to generate structured outputs like JSON, XML, or code following specific syntax rules. The core idea of constrained decoding is to limit the model at each decoding step to select only from a valid subset of tokens. For example, if the output must be valid JSON, the decoder dynamically builds a mask that allows only tokens valid at the current position. This ensures the generated output conforms to the required format from the start. Main approaches to constrained decoding include grammar-based methods, finite state machine (FSM) based methods, and token-tree based methods. The main challenge with constrained decoding is performance overhead. Building and applying token masks at each step adds computational overhead. However, modern implementations minimize this overhead through precompiling grammars into optimized data structures, using efficient bitmask operations, and caching intermediate states. In practice, constrained decoding typically adds only a small latency overhead (usually in the 5-15% range) while significantly improving output reliability and reducing post-processing requirements.
...
![图15-11 每个GPU只接收它为其托管的专家所需的token](../assets/images/ch15/fig15_11.png)
...
![图15-12 比较专家容量因子1.0与1.5](../assets/images/ch15/fig15_12.png)
...
![图15-13 自适应MoE路由的实际应用](../assets/images/ch15/fig15_13.png)
