...
您还可以使用带有默认TorchInductor后端的`torch.compile`来减少冗余复制，并在安全时融合相邻操作。`clone()`已经是设备到设备的复制，因此通常不会融合到单独的自定义内核中。启用自动调优（例如，`mode="max-autotune"`）以帮助TorchInductor在形状稳定时选择合并和向量化的调度。我们将在第13章和第14章深入介绍PyTorch编译器。
...
|--------|----------------------|-------------------|
| DRAM throughput (% of peak) | 25% | 90% (3.6x) |
| Global Memory Load Efficiency | 23% | 99% |
| Average sectors per request | 8.0 | 4.0 (optimal) |
| SM Active % | 62% | 99% |
| Kernel execution time (ms) | 4.8 ms | 1.3 ms (3.7x) |

注意：所有指标表中的数值均为说明概念而设的示例。有关不同GPU架构的实际基准测试结果，请参见GitHub仓库。
...
> Compared to an unaligned or strided case that can inflate sector counts, vectorized float4 loads reduce per-thread load instructions by 4x. This helps maintain the ideal 4.0 sectors per request when alignment is satisfied.

像PyTorch编译器这样的高级GPU编译器，当满足对齐和连续性要求时，通常可以生成向量化内存操作。您可以通过使用与向量宽度匹配的每线程tile来鼓励这种行为。因此，框架通常可以在底层实现类似的内存事务减少。但是，从一开始就正确对齐数据仍然是有益的。
...
> With 32-byte per-thread loads (e.g., 8 floats x 4 bytes per float), a warp moves 1,024 bytes (1,024 bytes = 32 threads per warp x 32 bytes per thread). This typically maps to 8 x 128-byte lines since each line consists of four 32-byte sectors. With 16-byte per-thread loads (e.g., four float4s), a warp moves 512 bytes (512 bytes = 32 threads per warp x 16 bytes per thread) or 4 x 128-byte lines. When the base pointer and per-thread stride are naturally aligned, the hardware coalescer combines lane accesses into full lines and avoids split sectors. This type of mechanical sympathy is crucial to achieving peak throughput. Therefore, for 256-bit loads on Blackwell, you should enforce 32-byte alignment to maintain peak performance.
...
这里我们使用CUDA的内置float4类型（来自`<cuda_runtime.h>`）。我们启动N/4个线程以保持16字节对齐并发出真正的向量加载/存储，每个线程复制一个float4（16字节）。这意味着每个线程处理四个float，每个线程束总共处理128个float，分成四个事务。`cudaMalloc`返回至少256字节对齐的指针，这满足float4（16字节）要求，并在您为数据起始地址使用32字节对齐时有助于32字节对齐向量。重要的是要注意，未对齐的转换可能会丧失向量化。以下是利用Blackwell对32字节向量化加载支持的示例：
...
> Here is an example that takes advantage of Blackwell's support for 32-byte vectorized loads:
...
Blackwell将每线程向量加载/存储的字节数相对于Hopper翻倍。在CUDA 13中，Blackwell可以执行32字节向量加载和存储，而不是Hopper上的16字节。这要求编译器能够证明32字节对齐。否则，它可能会将每线程加载/存储拆分为两条16字节指令。这将增加加载/存储相同数据量所需的指令数量，从而对性能产生负面影响。
...
| DRAM throughput (% of peak) | 25% | 90% (3.6x) |
| Kernel execution time | 4.2 ms | 1.2 ms (3.5x improvement) |
...
| Transactions/warp | 32 threads x 4 bytes → 128 bytes, equal to 4 32-byte sectors if aligned (4 sectors) | 32 threads x 16 bytes → 512 bytes, equal to 4 128-byte transactions, which correspond to 16 32-byte sectors (16 sectors) |
...
尽管Blackwell线程可以加载完整的32字节向量类型，但内存合并器仍然只能以128字节块或四个32字节扇区来服务请求。在Blackwell上，一个32线程的线程束每线程移动32字节，传输1024字节（8 x 128字节行）。Hopper的16字节/线程变体移动512字节（4 x 128字节）。请注意，事务数量随移动的字节数缩放。当正确对齐时，两者都完全高效：16字节（Hopper）或32字节（Blackwell及更高版本）。
...
> The result is a heavily memory-bound kernel with low SM utilization and frequent stalls waiting on global memory. Here is the PyTorch version of the naive matrix multiplication:
...
这段PyTorch代码故意写得非常慢，以说明在循环内执行冗余全局内存访问加载的极端情况。在实践中，像PyTorch这样的框架为此类操作使用优化的内核。
...
> The optimized version using shared memory to cache tiles for matrix multiplies is shown here:
...
这些示例专注于内存访问模式，使用的是FP32 CUDA核心——而不是降低精度的Tensor Core。第9章演示了使用降低精度计算（如16位、8位、4位）来进一步提高性能。
...
使用32 x 32分块时，当线程束中的线程都访问同一列时，它们会争用同一个共享内存bank。在实践中，您可以通过将分块填充到33列来避免这种情况——或者使用如交错等技术——使每次访问落入不同的bank。我们将在下一节应用此优化。目前，让我们专注于分块作为一种独立的优化。
...
|--------|----------------------|---------------------|-------|
| DRAM throughput (% of peak) | 90% | 25% (3.6x less) | Naive uses more bandwidth, but it's wasteful. The tile implementation is lower because it's more efficient after tiling due to fewer redundant loads. |
...
优化后的170 GFLOPS远低于Blackwell接近80 TFLOPS的理论FP32峰值。这对于小分块和频繁内存访问是预期的。重要的结果是在消除内存瓶颈后获得了11倍的改进。更大的问题或每字节更多的计算工作将使性能更接近峰值。
...
回想一下，每个线程最多可以使用255个寄存器。我们的分块内核实现保持在此限制内，避免了寄存器溢出，并保持了高性能。
...
如果您的内核使用额外的寄存器进行双缓冲寄存器、向量化加载等，您可以使用cudaOccupancyMaxPotentialBlockSize或Nsight Compute的占用率报告重新计算占用率。
...
像CUTLASS这样的库还包括性能分析器，可以自动化此探索过程，让您找到内核和硬件的最佳分块大小。
...
在实践中，您会使用像cuBLAS和PyTorch的torch.matmul这样的高性能库，它们已经在C++中实现了分块和其他优化。在生产代码中，直接使用torch.mm或torch.matmul会产生相同的好处——甚至可能更多，这要归功于高度调优的内核。
...
虽然您肯定可以重用现有的分块库和框架，但理解它们的工作原理（正如我们在这里所做的那样）对于诊断性能问题以及可能为这些库和框架未涵盖的专业情况编写自己的自定义内核是非常宝贵的。只是不要忘记回馈社区，因为他们给了您很多！
...
> As mentioned earlier in this section, when using a 32 x 32 tile, threads in a warp will contend for the same shared-memory bank when accessing the same column. Let's explain this problem-and some optimizations-in the next section.
...
> On modern NVIDIA GPUs (including Blackwell), shared memory has 32 banks, each 4 bytes wide (i.e., address mapping mod 32). As such, a warp striding by 128 B (32 floats x 4 B) will map all threads to the same bank. If multiple threads in a warp access the same bank, a bank conflict occurs. This forces serialization of memory accesses, which defeats the speed advantage of shared memory.

在代码中，当线程以导致它们落入同一个bank的跨步访问共享内存数组时，通常会发生bank冲突。图7-4显示了两个无bank冲突内存访问的示例。

> In code, bank conflicts typically occur when threads access a shared-memory array with a stride that causes them to land in the same bank. Figure 7-4 shows two examples of memory access without bank conflicts.
...
> Here, no two threads access the same memory bank at the same time. This is ideal. Figure 7-5 shows examples of 2-way and 16-way bank conflicts.
...
> Here, multiple threads are accessing the same memory bank, which will cause conflicts and impact performance. A classic example of bank conflicts is a naive matrix transpose using a 32 x 32 shared-memory tile. If 32 threads each read tile[i][threadIdx.x], such that the same column index (threadIdx.x) is read across different rows (i), all 32 threads in a warp will each access a memory address located in the same shared-memory bank, resulting in a 32-way bank conflict.

具体来说，在矩阵转置期间，您通过保持列索引不变（`threadIdx.x`）并跨线程变化行索引（i）来读取行主序分块的同一列。由于每行在内存中相隔128字节（32列 x 每列4字节 = 128字节），访问的内存地址将相差精确的128字节倍数。

> Specifically, during a matrix transpose, you read the same column of a row-major tile by keeping the column index constant (threadIdx.x) and varying the row index (i) across threads. Since each row is separated in memory by 128 bytes (32 columns x 4 bytes per column = 128 bytes), the memory addresses accessed will differ by exactly 128-byte multiples.

记住，地址到bank的映射每128字节重复一次，因为有32个bank，每个bank宽度为4字节（每次访问）。因此，访问偏移128字节的内存地址将总是落回bank 0--因此产生完整的32路bank冲突。

> Remember that the address-to-bank mapping repeats every 128 bytes because there are 32 banks, each 4 bytes wide (per access). Therefore, accessing memory addresses offset by 128 bytes will always fall back to bank 0-and thus produce a full 32-way bank conflict.

有一个值得注意的例外：如果线程束中的所有32个线程访问同一内存bank中的完全相同地址，硬件将在单个周期内将该值广播到所有线程。这避免了bank冲突。任何其他两个或多个不同内存地址访问同一bank的情况都会导致bank冲突并使内存访问串行化。
...
> Another common pitfall is using a stride equal to the number of memory banks, in this case 32. For example, if your index strides exactly 32 floats, each 4 bytes, then each thread's address ends up differing by multiples of 128 bytes. In this case, all threads map to bank 0, as shown in the following code:
...
> Here, threadIdx.x * 32 in floats becomes (threadIdx.x * 32 * 4) bytes. Since 32 * 4 = 128, each thread's memory load address is threadIdx.x * 128 bytes. And threadIdx.x * 128 mod 128 = 0 for all threads. They all hit bank 0 at the same time, creating a 32-way bank conflict.
...
> When this happens, the hardware must serialize what should have been 32 parallel reads into a sequence of single-bank accesses. In Nsight Compute (Shared Memory section), you'll see bank-conflict counts increase and shared memory efficiency decrease. Meanwhile, shared memory throughput is only a fraction of its expected bandwidth. In Nsight Systems, you'll see warps stalled for long periods on bank conflicts instead of doing useful work.

Bank冲突强制本应并行的片上共享内存访问一个接一个地重放，消除了您期望从缓冲获得的任何加速，并往往产生令人失望的、低于预期的性能。如果您的内核没有按预期加速，bank冲突是一个可能的原因。

> Bank conflicts force what should have been parallel on-chip shared-memory accesses to replay one after another, wiping out any speedup you expected from buffering and often resulting in disappointing, lower-than-expected performance. If your kernel is not accelerating as expected, bank conflicts are a likely culprit.

始终选择您的跨步和数据布局，使同一线程束中的线程命中不同的bank，避免那种串行化瓶颈。
...
> Padding adds negligible overhead, about 3% more memory for a 32-wide tile, but it completely eliminates conflicts, which dramatically improves performance. Remember that Blackwell has > 200 KB shared memory per SM. A 3% memory overhead for a 32 x 32 tile is only 1 KB. This is well worth the performance gain.

让我们展示一个为简单转置内核移除共享内存bank冲突的示例。在这个示例中，每个线程访问与其他线程落入同一共享内存bank的共享内存地址。这导致内存访问串行化，阻止并行性，并显示缓慢的性能。以下是产生bank冲突的转置内核的朴素实现。

> Let's demonstrate removing shared-memory bank conflicts for a simple transpose kernel. In this example, each thread accesses a shared-memory address that falls into the same shared-memory bank as other threads. This causes memory access serialization, prevents parallelism, and shows slow performance. Following is the naive implementation of a transpose kernel that produces bank conflicts.
...
请记住，Blackwell的共享内存有32个bank。这个数字正好与线程束中的线程数量32相匹配。因此，如果所有线程索引到同一个bank（当您固定行并改变列时会发生这种情况），您将始终获得完整的32路冲突。这种一对一对应意味着在线程束粒度上的任何未对齐访问模式都将强制每个线程串行化通过同一个bank——无论GPU的架构进步如何。
...
> After example (C++): padded transpose (avoiding bank conflicts). Here, we add a small padding, one extra unused column, so that each row of shared memory starts at a different bank modulus. This way, bank index conflicts are eliminated by the offset:
...
PyTorch本身不暴露任何用于共享内存填充的高级API，因此您必须在CUDA内核中手动实现这些，并使用torch.utils.cpp_extension加载它们。然而，在实践中，PyTorch依赖于像cuDNN和cuBLAS这样的优化库，它们在底层实现了避免bank冲突和最大化吞吐量的技术。
...
| Shared memory utilization (% of peak) | 52% | 100% |
| Warp stall fraction (memory) | 38% | 0.5% |
| Kernel execution time (ms) | 4 ms | 1.3 ms (3x improvement) |
...
NVIDIA的CUTLASS库和其他高性能基于CUDA的库在其tile迭代器中使用索引swizzling，以确保线程映射到不同的bank，避免bank冲突，并优化共享内存使用。
...
> Next, let's explore a technique that avoids shared memory entirely and communicates directly between threads.
...
现代GPU会自动广播单个共享内存值，如果所有线程访问完全相同的内存地址。这将在这个特殊的单地址情况下避免bank冲突。另一方面，线程束shuffle使用广播来避免更一般的、任意的多值数据访问模式的bank问题。
...
> Using __shfl_sync, you can perform a tree reduction entirely in registers. For example, let's use a convenient variant called __shfl_down_sync to perform a butterfly-style reduction that has each thread read the value held by another thread offset by a certain number of lanes, as shown here:
...
Shuffle仅限于单个线程束内。对于现代GPU架构上的线程束间交换，还可以考虑使用协作组进行线程块级别的归约，或在支持时使用线程块集群级别的通信。我们将在第10章讨论这些概念。
...
请注意，我们这里所说的只读缓存与之前GPU内存层次结构部分讨论的64 KB常量内存缓存不同。它是一个用于不可变数据的大型缓存。另一方面，常量内存缓存对于大型数组来说太小了。现代架构依赖于更大的L1/只读缓存来进行这类嵌入向量查找——而不是试图将这些数据塞进小的64 KB常量内存缓存中。
...
在现代CUDA中，您通常不需要显式调用`__ldg()`。如果指针是`const __restrict__`，编译器可以在提供安全性时为全局内存加载使用只读数据缓存。较旧的`__ldg()`内建函数仍然可用于显式控制，但在现代编译器中通常不需要。
...
> As an example of the caching advantage of using the standard read-only cache, assume we have a kernel that, for each thread, looks up a value from a table of size T = 1,024 and writes it to output. This simulates an embedding lookup pattern, and following is the implementation in both CUDA C++ and PyTorch:
...
在PyTorch环境中，您可以通过使用`torch.utils.cpp_extension`编写一个小型CUDA扩展来实现相同的技巧，该扩展将您的嵌入表指针声明为`const __restrict__`。您也可以使用PyTorch编译器来优化此代码。我们将在第13章和第14章深入介绍PyTorch编译器。
...
> Table 7-6. Benefits of the read-only cache

| Metric | Before (no __restrict__) | After (with __restrict__) |
|--------|--------------------------|---------------------------|
...
Nsight Systems呈现整体GPU活动的时间线视图，而Nsight Compute报告每个内核的指标，如SM活跃度%。当您需要定量的逐内核分析时，请使用Nsight Compute。
...
虽然纹理引用和表面引用API已被弃用，但纹理对象和表面对象API仍然受支持，适用于具有二维或三维局部性的访问模式。然而，对于大多数涉及1D数据的AI工作负载，使用只读数据缓存配合常量内存更简单且更受推荐。
...
TMA是在全局内存和SMEM/TMEM之间移动2D和N维tile时进行分块批量复制的首选路径。在块作用域中优先使用`cuda::memcpy_async`配合`cuda::pipeline`；在Hopper/Blackwell上，当对齐和方向允许时（例如全局内存↔共享内存），实现将利用TMA（`cp.async.bulk.*`系列）。
...
> Table 7-7. Comparing naive kernel (no prefetch) vs. TMA-accelerated double-buffering
...
对于几乎所有情况，您应该使用可用的最高级别和最新的API来编写CUDA内核进行全局到共享内存的tiling。这包括CUDA流水线API（`cuda::memcpy_async`）。这些API和库不断改进，并将透明地利用最新的硬件功能，如TMA进行批量、跨步和2D/3D传输。此外，它们还支持高级性能优化功能，如线程块集群的多播。使用这些API时，您可以"免费"获得所有这些功能——无需更改代码。
