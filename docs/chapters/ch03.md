...
*图3-1 用于开发和生产化现代LLM工作负载的常见框架、库、编译器、运行时和工具集*
...
*图3-2 使用nvcc将CUDA程序编译为PTX--最终编译为GPU目标设备的低级指令*
...
*图3-5 固定内存（也称为页锁定或不可分页）是一种不能交换到磁盘的内存类型*
...
![图3-6 进程A和B在没有MPS的情况下启动内核时的GPU调度](../assets/images/ch03/fig03_131_1.png)

*图3-6 进程A和B在没有MPS的情况下启动内核时的GPU调度*
...
![图3-7 使用MPS减少进程A和B的空闲间隙](../assets/images/ch03/fig03_131_2.png)
