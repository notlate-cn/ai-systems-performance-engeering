...
![图11-1 从CPU向GPU上运行的两个流启动五个内核](../assets/images/ch11/fig11_01.png)
...
![图11-2 三重重叠时间线](../assets/images/ch11/fig11_02.png)
...
![图11-3 流有序内存分配](../assets/images/ch11/fig11_03.png)
...
![图11-4 显示多个GPU内核在启用PTDS的情况下跨不同线程的各自默认流上的单独CUDA流并发运行的时间线](../assets/images/ch11/fig11_04.png)
...
![图11-5 使用cudaStreamSynchronize()将阻塞CPU，直到流中的所有操作都已同步](../assets/images/ch11/fig11_05.png)
...
![图11-6 使用CUDA事件以细粒度方式进行同步](../assets/images/ch11/fig11_06.png)
...
![图11-7 使用CUDA事件同步流（来源：https://oreil.ly/MynOA）](../assets/images/ch11/fig11_07.png)
...
![图11-8 四个GPU在环形中的分块all-reduce](../assets/images/ch11/fig11_08.png)
...
![图11-9 使用PDL从内核A启动内核B——部分重叠内核B的执行与内核A的收尾阶段（在同一个CUDA流中）](../assets/images/ch11/fig11_09.png)
...
![图11-10 使用PDL和线程块集群以及TMA多播的线程束专用化多级流水线，用于高性能的内核间和内核内GEMM实现](../assets/images/ch11/fig11_10.png)
...
![图11-11 TMA多播：领导者CTA向DSMEM（集群共享内存）发出cp.async.bulk.tensor；跟随者CTA使用cluster.map_shared_rank消费瓦片；cuda::memcpy_async驱动TMA](../assets/images/ch11/fig11_11.png)
...
在多GPU上下文中，跨不同流重叠点对点传输、集合通信和计算进一步最小化了空闲时间，并确保每个GPU的硬件资源被充分利用。这些技术——结合对资源限制和同步点的理解——构成了高性能AI系统的基础。

> In multi-GPU contexts, overlapping peer-to-peer transfers, collective communications, and computations across distinct streams further minimizes idle time and ensures that each GPU's hardware resources are fully utilized. These techniques—combined with an understanding of resource limits and synchronization points—form the foundation of high-performance AI systems.

我们回顾了第10章，展示了如何使用CUDA图捕获这些流工作流，以减少重复迭代的CPU开销。

> We looked back at Chapter 10 by showing how to capture these stream workflows using CUDA Graphs to reduce CPU overhead for repeated iterations.
